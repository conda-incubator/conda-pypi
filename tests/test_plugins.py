...
def test_extract_whl_as_conda_called(tmp_env: TmpEnvFixture, conda_cli: CondaCLIFixture, mocker: MockerFixture, pypi_demo_package_wheel_path: Path):
    
    spy=mocker.spy(extract_whl_or_tarball, "extract_whl_or_tarball")
    with tmp_env("python") as prefix:
        conda_cli("install", WHL_URL,  "-p", prefix)
