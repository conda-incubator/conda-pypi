...
def test_extract_whl_as_conda_called(
    ...
    with tmp_env("python") as prefix:
        ...
        conda_cli("install", package, "-p", prefix)
        spy.assert_called_once()


def test_extract_whl_as_conda_not_called(
    tmp_env: TmpEnvFixture,
    conda_cli: CondaCLIFixture,
    mocker: MockerFixture,
    tmp_pkgs_dir: Path,
    ...
    with tmp_env("python") as prefix:
        spy = mocker.spy(extract_whl_or_tarball.extract_whl, "extract_whl_as_conda_pkg")
        conda_cli("install", "numpy", "-p", prefix, "--yes")
        spy.assert_not_called()


def test_plugin_invoked(
    tmp_env: TmpEnvFixture,
    conda_cli: CondaCLIFixture,
    mocker: MockerFixture,
    pypi_demo_package_wheel_path: Path,
